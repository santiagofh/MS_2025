...
directory_2025 = r"C:\Users\fariass\OneDrive - SUBSECRETARIA DE SALUD PUBLICA\Escritorio\DATA\REM\REM_2025"
...
#%% -----------------------------------------------------------------------------------------
# Lectura FONASA
fonasa1 = pd.read_excel('FONASA/T8009_Inscritos_RM.xlsx', skiprows=3)
fonasa1
#%%
fonasa = pd.concat([fonasa1], ignore_index=True)
...
# ------------------------------------------------------------------------------
# META I (MSI)
def calcular_MSI(df_rem_2024, df_rem_2025, region_id):
    """
    ...
    df_2024_oct_to_dec = df_rem_2024.loc[df_rem_2024['Mes'] >= 10].copy()
    
    # 2) Filtramos df_rem_2024 sólo para meses <= 9 (Ene hasta Sep)
    df_2025_jan_to_sep = df_rem_2025.loc[df_rem_2025['Mes'] <= 9].copy()
    
    # 3) Concatenamos ambos
    df_combined = pd.concat([df_2024_oct_to_dec, df_2025_jan_to_sep], ignore_index=True)
...
df_MSI = calcular_MSI(df_rem_2024, df_rem_2025, region_id)
df_MSII  = calcular_MSII(df_rem_2025, fonasa_rm, region_id)
df_MSIIIa= calcular_MSIIIa(df_rem_2025, fonasa_rm, region_id)
df_MSIIIb= calcular_MSIIIb(df_rem_2025, fonasa_rm, region_id)
df_MSIVa = calcular_MSIVa(df_rem_2025, fonasa_rm, region_id)
df_MSIVb = calcular_MSIVb(df_rem_2025, region_id)
df_MSV   = calcular_MSV(df_rem_2025, fonasa_rm, region_id)
df_MSVI  = calcular_MSVI(df_rem_2025, region_id)
df_MSVII = calcular_MSVII(df_rem_2025, fonasa_rm, region_id)
...
df_final2.to_csv('MS2025_v2.csv', index=False, encoding='utf-8')
print("Archivo CSV guardado exitosamente: 'MS2025_v2.csv'")
#%% -----------------------------------------------------------------------------------------
